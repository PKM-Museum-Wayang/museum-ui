...
interface KegiatanEditModalProps {
  id: number
  onClose: () => void
  onSuccess: () => void
}
...
export default function KegiatanEditModal({ id, onClose, onSuccess }: KegiatanEditModalProps) {
  ...
  useEffect(() => {
    api.get(`/kegiatan/${id}`)
      .then(res => {
        const k: KegiatanDetail = res.data.data
        const tanggalObj = new Date(k.tanggal)

        // yyyy-mm-dd dan HH:mm sesuai timezone lokal, bukan getISO*
        // (yang selalu UTC dan bisa mundur/maju satu hari/jam)
        const pad = (n: number) => String(n).padStart(2, '0')

        setForm({
          nama: k.nama,
          tanggal: `${tanggalObj.getFullYear()}-${pad(tanggalObj.getMonth() + 1)}-${pad(tanggalObj.getDate())}`,
          jam: `${pad(tanggalObj.getHours())}:${pad(tanggalObj.getMinutes())}`,
          lokasi: k.lokasi,
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
        <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md text-center text-slate-400 text-sm">
          Memuat data…
        </div>
      </div>
    )
